  ...
  tbody.innerHTML = sortedAgents
    .map((a) => {
      ...
      return `<tr onclick="drillToAgent(${a.id})">
        <td><div class="agent-name-cell"><div class="agent-mini-avatar">${initials(a.name)}</div><div><div style="font-weight:600">${a.name} ${a.is_transferred ? '<span class="days-badge warn" style="font-size:9px;padding:2px 4px;margin-left:6px;display:inline-flex;">No longer in dept</span>' : ''}</div><div style="font-size:10px;color:var(--grey-400)">${a.designation}</div></div></div></td>
        <td>${a.leads_offplan}</td>
        <td>${a.leads_secondary}</td>
        <td>${a.reshuffled_leads}</td>
        <td>${a.deals}</td>
